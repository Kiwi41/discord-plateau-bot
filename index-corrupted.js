...
const config = {
    ...
    registrationUrl: process.env.REGISTRATION_URL || 'https://example.com/inscription',    // Commande po    // Commande pour traiter les 4 prochains vendredis
    ...
    }er/mettre à jour un post pour le prochain vendredi
    ...
    if (message.content === '!process-next-month' || message.content === '!plateau-next-month') {
        ...
        await processNextFourFridays();
        message.channel.send('✅ Traitement des 4 prochains vendredis terminé!');
    }ande pour traiter les 4 prochains vendredis
    if (message.content === '!process-next-month' || message.content === '!plateau-next-month') {
        message.reply('📅 Traitement des 4 prochains vendredis en cours...');
        await processNextFourFridays();
        message.followUp('✅ Traitement des 4 prochains vendredis terminé!');
    }entId: process.env.EVENT_ID,
...
async function processOneFriday(guild, forumChannel, fridayDate, allEvents = null) {
    ...
    let recurringEventData = null; // Pour stocker l'événement récurrent
        ...
        if (fridayEvent) {
            ...
            // Debug: affichage des informations de l'événement (désactivé en production)
            // console.log(`🔍 Debug événement trouvé:`);
            // console.log(`   Nom: ${fridayEvent.name}`);
            // console.log(`   Type d'entité: ${fridayEvent.entityType}`);
...
client.once('clientReady', () => {
    ...
    console.log('📝 Commandes manuelles (admin):');
    ...
    // Test de création de post (optionnel, pour tester) - Désactivé pour utiliser les commandes manuelles
    // console.log('🧪 Test corrigé des 4 prochains vendredis dans 5 secondes...');
    // setTimeout(() => processNextFourFridays(), 5000);
});
...
client.on('messageCreate', async (message) => {
    ...
    console.log('✅ Utilisateur détecté, traitement de la commande... (mode test)');
    ...
    // Commande pour traiter les 4 prochains vendredis
    if (message.content === '!process-next-month') {
        message.reply('� Traitement des 4 prochains vendredis en cours...');
        await processNextFourFridays();
        message.followUp('✅ Traitement des 4 prochains vendredis terminé!');
    }
    ...
    if (message.content === '!plateau-help') {
        const helpEmbed = new EmbedBuilder()
            ...
            .setDescription('Commandes disponibles pour les administrateurs:')
            .addFields(
